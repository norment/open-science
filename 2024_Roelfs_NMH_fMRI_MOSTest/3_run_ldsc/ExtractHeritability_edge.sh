...
LOGDIR="${PROJECTDIR}/ldsc/ldsc_out_edge"

rm LDSC_h2_edge.txt

echo -e "calc\tedge\th2\tse\tLambdaGC\tintercept\tintercept_se" | tee LDSC_h2_edge.txt

for i in `ls ${LOGDIR}/h2_*.log`; do

	CALC=`basename $i | awk -F _ '{print $2}'`
	EDGE=`basename $i | awk -F _ '{print $3}' | awk -F . '{print $1}'`
  ...
  echo -e "${CALC}\t${EDGE}\t${h2}\t${se}\t${lGC}\t${int}\t${int_se}" | tee -a LDSC_h2_edge.txt
